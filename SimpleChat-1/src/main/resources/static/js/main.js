...
function connect(event) {
	...
	if (name) {
		...
		//serguei3000 Инициализируем 2 библиотеки: SockJS и Stomp
		var socket = new SockJS('/websocketApp');
		stompClient = Stomp.over(socket);
		
		
		//serguei3000 Подключаем обработчик
...
function connectionSuccess() {
	...
	//serguei3000 Подключаем обработчик
	//serguei3000 адресация с "topic" направляется брокеру, в нашем случае это рэббит
	stompClient.subscribe('/topic/javainuse', onMessageReceived);

	//serguei3000 адресация с "app" направляется контроллеру
